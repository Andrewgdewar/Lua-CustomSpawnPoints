...
class Mod implements IPostDBLoadMod {
  ...
  public postDBLoad(container: DependencyContainer): void {
    ...
    logger.info(
      `Loading: ${this.Name} ${pkg.version}${
        modConfig.Enabled === true ? "" : " [Disabled]"
      }`
    );
    ...
    this.Path = `${dirArray[dirArray.length - 4]}/${
      dirArray[dirArray.length - 3]
    }/${dirArray[dirArray.length - 2]}`; // Pon, Pin, Fon, Fin

    this.GetToWork();

    container
      .resolve<StaticRouterModService>("StaticRouterModService")
        ...
        [
          {
            ...
            action: (
              ...
              return this.container
                .resolve<LocationCallbacks>("LocationCallbacks")
  ...
  public GetToWork() {
    ...
    let totalSpawnPoints = {
      Default: { Bot: 0, Player: 0, Boss: 0 },
      Custom: { Bot: 0, Player: 0, Boss: 0 },
    };

    // Check Disabled Folders List
    let disableList =
      modConfig.DisableFolderList !== undefined &&
      modConfig.DisableFolderList.trim().length > 0
        ? modConfig.DisableFolderList.split(",").map((item) => item.trim())
        : undefined;
    for (let folderIndex = 0; folderIndex < dirsList.length; folderIndex++) {
      // Remove Empty Folder
      if (
        vfs.getFiles(`${spawnPointsPath}${dirsList[folderIndex]}`).length === 0
      ) {
      ...
      for (let index in disableList) {
        if (
          disableList[index].toLowerCase() ===
          dirsList[folderIndex].toLowerCase()
        ) {
          dirsList.splice(folderIndex--, 1);
          break;
        }
    ...
    for (let map in databaseServer.locations) {
      ...
      let mapBase = databaseServer.locations[map].base as ILocationBase;
      ...
      let newSpawnPointParams = [] as SpawnPointParam[];
      ...
      if (
        this.defaultSpawnParms[map] === undefined &&
        mapBase.SpawnPointParams !== undefined
      ) {
      ...
      if (this.defaultSpawnParms[map] !== undefined) {
        ...
        for (let key = 0; key < newSpawnPointParams.length; key++) {
          let values = newSpawnPointParams[key] as SpawnPointParam;
          for (
            let categoryIndex = 0;
            categoryIndex < values.Categories.length;
            categoryIndex++
          ) {
            switch (values.Categories[categoryIndex]) {
              case "Bot": {
                if (
                  (modConfig.DisableDefaultSpawnPoints.Bot[map] || false) ===
                  true
                ) {
              ...
              case "Player": {
                if (
                  (modConfig.DisableDefaultSpawnPoints.Player[map] || false) ===
                  true
                ) {
                  values["Infiltration"] = "";
                  values.Categories.splice(categoryIndex--, 1);
              ...
              case "Boss": {
                if (
                  (modConfig.DisableDefaultSpawnPoints.Boss[map] || false) ===
                  true
                ) {
                  values.Categories.splice(categoryIndex--, 1);
                } else {
      ...
      for (let folderIndex in dirsList) {
        let filePath =
          Number(folderIndex) > 0
            ? `${spawnPointsPath}${dirsList[folderIndex]}/${map}.json`
            : `${spawnPointsPath}${map}.json`;
        let config =
          vfs.exists(filePath) === true
            ? jsonUtil.deserialize(vfs.readFile(filePath))
            : undefined;
        ...
        if (config === undefined) {
          ...
          for (let [_, values] of Object.entries(mapBase.waves)) {
            if (
              values["SpawnPoints"] === undefined ||
              values["SpawnPoints"].length < 1
            ) {
              continue;
            }

            if (
              spawnZones.findIndex((item) => item === values["SpawnPoints"]) ===
              -1
            ) {
          ...
          for (let [_, values] of Object.entries(mapBase.BossLocationSpawn)) {
            if (
              values["BossZone"] === undefined ||
              values["BossZone"].length < 1
            ) {
              continue;
            }
            ...
            if (values["BossZone"].includes(",") === false) {
              if (
                spawnZones === undefined ||
                spawnZones.findIndex((item) => item === values["BossZone"]) ===
                  -1
              ) {
                spawnZones.push(values["BossZone"]);
              }
            } else {
              let arr = values["BossZone"]
                .split(",")
                .map((item) => item.trim());
          ...
          for (let [_, values] of Object.entries(this.defaultSpawnParms[map])) {
            ...
            if (
              values["Infiltration"] !== undefined &&
              values["Infiltration"].length > 1 &&
              infiltrationsArr.findIndex(
                (item) => item === values["Infiltration"]
              ) === -1
            ) {
            ...
            // Spawn Zone
            if (
              values["BotZoneName"] !== undefined &&
              values["BotZoneName"].length > 0 &&
              spawnZones.findIndex((item) => item === values["BotZoneName"]) ===
                -1
            ) {
          ...
          if (lists.length > 0) {
            ...
            logger.log(
              `${this.Name}: Config Created "${filePath.replace(
                "user/mods/Lua-CustomSpawnPoints/",
                ""
              )}" `,
              LogTextColor.YELLOW,
          ...
          if (config["SpawnZone Available List"] === undefined) {
            logger.error(
              `${this.Name}: ${map} has no "SpawnZone Available List", delete the config file to re-generate the file... `
            );
            continue;
          }
          ...
          if (config["Infiltration Available List"] === undefined) {
            logger.error(
              `${this.Name}: ${map} has no "Infiltration Available List", delete the config file to re-generate the file... `
            );
            continue;
          }

          let spawnZone = config["SpawnZone Available List"]
            .split(",")
            .map((item) => item.trim());
          let infiltrations = config["Infiltration Available List"]
            .split(",")
            .map((item) => item.trim());
          for (let [zoneName, values] of Object.entries(config)) {
            if (
              zoneName === "SpawnZone Available List" ||
              zoneName === "Infiltration Available List" ||
            ...
            if (foundZone === false) {
              if (spawnZone.length === 1) {
                ...
                logger.warning(
                  `${this.Name}: ${map} has non-exist spawn zone "${zoneName}", Fixed to "${spawnZone[0]}" `
                );
              } else {
                logger.error(
                  `${this.Name}: ${map} has non-exist spawn zone "${zoneName}", Skipping... `
                );
                continue;
              }
            }
            ...
            for (let index in config[zoneName]) {
              ...
              if (zoneConfig["Infiltration"].length <= 1) {
                for (let index in zoneConfig.Categories) {
                  if (zoneConfig.Categories[index] === "Player") {
                    if (infiltrations.length === 1) {
                      ...
                      logger.warning(
                        `${this.Name}: ${map} has "${zoneConfig.Categories[index]}" spawn zone in "${zoneName}" but [Infiltration] has empty value, Fixed value to "${infiltrations[0]}" `
                      );
                    } else {
                      logger.error(
                        `${this.Name}: ${map} has "${zoneConfig.Categories[index]}" spawn zone in "${zoneName}" but [Infiltration] has empty value, Skipping... `
                      );
                      bSkip = true;
                      break;
                    }
                ...
                if (foundZone === false) {
                  logger.error(
                    `${this.Name}: ${map} has non-exist infiltration "${zoneConfig["Infiltration"]}" in "${zoneName}", Skipping... `
                  );
                  continue;
                }
              }
              ...
              if (
                zoneConfig.Position === undefined ||
                typeof zoneConfig.Sides !== "object" ||
                zoneConfig.Position.length !== 3
              ) {
                logger.error(
                  `${this.Name}: ${map} has bad Position values [${zoneConfig.Position[0]}, ${zoneConfig.Position[1]}, ${zoneConfig.Position[2]}] in "${zoneName}", Skipping... `
                );
                continue;
              }

              if (
                zoneConfig.Rotation === undefined ||
                typeof zoneConfig.Rotation !== "number" ||
                zoneConfig.Rotation < 0 ||
                zoneConfig.Rotation > 360
              ) {
                logger.error(
                  `${this.Name}: ${map} has bad Rotation value ${zoneConfig.Rotation} in "${zoneName}", Fixed value to 90.0 `
                );
              ...
              if (
                zoneConfig.Sides === undefined ||
                typeof zoneConfig.Sides !== "object"
              ) {
                logger.error(
                  `${this.Name}: ${map} has bad Sides value [${zoneConfig.Sides}] in "${zoneName}", Skipping... `
                );
                continue;
              }

              if (
                zoneConfig.Categories === undefined ||
                typeof zoneConfig.Categories !== "object"
              ) {
                logger.error(
                  `${this.Name}: ${map} has bad Categories value [${zoneConfig.Categories}] in "${zoneName}", Skipping... `
                );
                continue;
              }

              if (
                zoneConfig.DelayToCanSpawnSec === undefined ||
                typeof zoneConfig.DelayToCanSpawnSec !== "number" ||
                zoneConfig.DelayToCanSpawnSec < 0
              ) {
                logger.error(
                  `${this.Name}: ${map} has bad DelayToCanSpawnSec value ${zoneConfig.DelayToCanSpawnSec} in "${zoneName}", Fixed value to 1 `
                );
                zoneConfig.DelayToCanSpawnSec = 1;
              }

              if (
                zoneConfig.ColliderRadius === undefined ||
                typeof zoneConfig.ColliderRadius !== "number" ||
                zoneConfig.ColliderRadius < 0
              ) {
                logger.error(
                  `${this.Name}: ${map} has bad ColliderRadius value ${zoneConfig.ColliderRadius} in "${zoneName}", Fixed value to 0.0 `
                );
              ...
              for (let index in zoneConfig.Categories) {
                switch (zoneConfig.Categories[index]) {
                  case "Bot":
                    currentSpawnPoints.Custom.Bot++;
                    break;
                  case "Player":
                    currentSpawnPoints.Custom.Player++;
                    break;
                  case "Boss":
                    currentSpawnPoints.Custom.Boss++;
                }
              }
              if (modConfig.DEBUG.PrintAddedSpawnPoints === true) {
                logger.debug(
                  `${dirsList[folderIndex]}: ${map} @ "${zoneName}" [${zoneConfig.Position[0]}, ${zoneConfig.Position[1]}, ${zoneConfig.Position[2]}] for "${zoneConfig.Sides}" and "${zoneConfig.Categories}" with ${zoneConfig.ColliderRadius} ColliderRadius\nComment: "${zoneConfig.Comment}" `
                );
              }
            }
          }
      ...
      if (foundSpawn.includes(false) === true) {
        if (foundSpawn[0] === false)
          logger.error(
            `${this.Name}: No spawn point for "Bot" in "${map}"... `
          );
        if (foundSpawn[1] === false)
          logger.error(
            `${this.Name}: No spawn point for "Player" in "${map}"... `
          );
        ...
        for (let key in this.defaultSpawnParms[map]) {
          ...
          for (let newKey in newSpawnPointParams) {
            if (
              this.defaultSpawnParms[map][key].Id ===
              newSpawnPointParams[newKey].Id
            ) {
          ...
          if (currentSpawnPointParms === undefined) {
            ...
            if (
              foundSpawn[0] === false &&
              currentSpawnPointParms.Categories.includes("Bot") === true
            ) {
              ...
              if (
                (modConfig.DisableDefaultSpawnPoints.Boss[map] || false) ===
                  false &&
                currentSpawnPointParms.Categories.includes("Boss") === true
              ) {
            ...
            if (
              foundSpawn[1] === false &&
              currentSpawnPointParms["Infiltration"].length > 1 &&
              currentSpawnPointParms.Categories.includes("Player") === true
            ) {
            ...
            if (
              foundSpawn[0] === false &&
              defaultSpawnPoint.Categories.includes("Bot") === true
            ) {
              ...
              if (
                (modConfig.DisableDefaultSpawnPoints.Boss[map] || false) ===
                  false &&
                currentSpawnPointParms.Categories.includes("Boss") === false &&
                defaultSpawnPoint.Categories.includes("Boss") === true
              ) {
            ...
            if (
              foundSpawn[1] === false &&
              defaultSpawnPoint["Infiltration"].length > 1 &&
              defaultSpawnPoint.Categories.includes("Player") === true
            ) {
              currentSpawnPointParms.Categories.push("Player");
              currentSpawnPointParms["Infiltration"] =
                defaultSpawnPoint["Infiltration"];
              currentSpawnPoints.Default.Player++;
            }
          }
        }
        ...
        if (foundSpawn[0] === false) {
          if (currentSpawnPoints.Default.Bot === 0) {
            logger.error(
              `${this.Name}: No spawn point for "Bot" in default spawn points list, Skipping...\n  >>  (You have to add Custom or Default Spawn Points) `
            );
          } else {
            foundSpawn[0] = true;
            logger.warning(
              `${this.Name}: Recovered "Bot" default spawn points: ${currentSpawnPoints.Default.Bot} `
            );
          }
        }
        ...
        if (foundSpawn[1] === false) {
          if (currentSpawnPoints.Default.Player === 0) {
            logger.error(
              `${this.Name}: No spawn point for "Player" in default spawn points list, Skipping...\n  >>  (You have to add Custom or Default Spawn Points) `
            );
          } else {
            foundSpawn[1] = true;
            logger.warning(
              `${this.Name}: Recovered "Player" default spawn points: ${currentSpawnPoints.Default.Player} `
            );
          }
        }
      ...
      if (modConfig.PrintMapLoadLogs === true) {
        logger.log(
          `[${map}]${this.GetSpace(map, 16)}Custom Spawn Points      Bot: ${
            currentSpawnPoints.Custom.Bot
          },${this.GetSpace(currentSpawnPoints.Custom.Bot, 5)}Player: ${
            currentSpawnPoints.Custom.Player
          },${this.GetSpace(currentSpawnPoints.Custom.Player, 5)}Boss: ${
            currentSpawnPoints.Custom.Boss
          } `,
          LogTextColor.WHITE,
        ...
        logger.log(
          `[${map}]${this.GetSpace(map, 16)}Default Spawn Points     Bot: ${
            currentSpawnPoints.Default.Bot
          },${this.GetSpace(currentSpawnPoints.Default.Bot, 5)}Player: ${
            currentSpawnPoints.Default.Player
          },${this.GetSpace(currentSpawnPoints.Default.Player, 5)}Boss: ${
            currentSpawnPoints.Default.Boss
          } \n`,
          LogTextColor.WHITE,
          LogBackgroundColor.CYAN
    ...
    if (updatedList.length > 0) {
      logger.log(
        `${this.Name}: Custom Spawn Points    Bot: ${
          totalSpawnPoints.Custom.Bot
        },${this.GetSpace(totalSpawnPoints.Custom.Bot, 5)}Player: ${
          totalSpawnPoints.Custom.Player
        },${this.GetSpace(totalSpawnPoints.Custom.Player, 5)}Boss: ${
          totalSpawnPoints.Custom.Boss
        } `,
        LogTextColor.WHITE,
        LogBackgroundColor.BLUE
      );
      logger.log(
        `${this.Name}: Default Spawn Points   Bot: ${
          totalSpawnPoints.Default.Bot
        },${this.GetSpace(totalSpawnPoints.Default.Bot, 5)}Player: ${
          totalSpawnPoints.Default.Player
        },${this.GetSpace(totalSpawnPoints.Default.Player, 5)}Boss: ${
          totalSpawnPoints.Default.Boss
        } `,
        LogTextColor.WHITE,
        LogBackgroundColor.BLUE
      );
      logger.log(
        `${this.Name}: Successfully Loaded ${
          dirsList.length - 1
        } Presets for "${updatedList}" `,
        LogTextColor.WHITE,
        LogBackgroundColor.BLUE
      );
    } else {
    ...
    if (readMe) {
      logger.log(
        `${this.Name}: Check out "!Readme/example.json" for how to make Custom Spawn Points! `,
        LogTextColor.WHITE,
