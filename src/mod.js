...
class Mod {
    Path;
    Name = `${package_json_1.default.author}-${package_json_1.default.name}`;
    container;
    defaultSpawnParms = {};
    postDBLoad(container) {
        ...
        logger.info(`Loading: ${this.Name} ${package_json_1.default.version}${config_json_1.default.Enabled === true ? "" : " [Disabled]"}`);
        ...
        this.GetToWork();
        container
            .resolve("StaticRouterModService")
            .registerStaticRouter(`${this.Name}-/client/locations`, [
            {
                url: "/client/locations",
                action: (url, info, sessionID, output) => {
                    this.GetToWork();
                    return this.container
                        .resolve("LocationCallbacks")
    ...
    GetToWork() {
        ...
        let disableList = config_json_1.default.DisableFolderList !== undefined &&
            config_json_1.default.DisableFolderList.trim().length > 0
            ? config_json_1.default.DisableFolderList.split(",").map((item) => item.trim())
            : undefined;
        for (let folderIndex = 0; folderIndex < dirsList.length; folderIndex++) {
            ...
            for (let index in disableList) {
                if (disableList[index].toLowerCase() ===
        ...
        for (let map in databaseServer.locations) {
            ...
            // DEBU9
            if (config_json_1.default.DEBUG.UnlimitedRaidTime === true) {
                ...
                for (let key = 0; key < newSpawnPointParams.length; key++) {
                    ...
                    for (let categoryIndex = 0; categoryIndex < values.Categories.length; categoryIndex++) {
                        switch (values.Categories[categoryIndex]) {
                            case "Bot": {
                                if ((config_json_1.default.DisableDefaultSpawnPoints.Bot[map] || false) ===
                                    true) {
                            ...
                            case "Player": {
                                if ((config_json_1.default.DisableDefaultSpawnPoints.Player[map] || false) ===
                                    true) {
                                    values["Infiltration"] = "";
                                    values.Categories.splice(categoryIndex--, 1);
                                }
                            ...
                            case "Boss": {
                                if ((config_json_1.default.DisableDefaultSpawnPoints.Boss[map] || false) ===
                                    true) {
                                    values.Categories.splice(categoryIndex--, 1);
                                }
                                else {
            ...
            for (let folderIndex in dirsList) {
                ...
                if (config === undefined) {
                    ...
                    for (let [_, values] of Object.entries(mapBase.waves)) {
                        if (values["SpawnPoints"] === undefined ||
                            values["SpawnPoints"].length < 1) {
                            continue;
                        }
                        if (spawnZones.findIndex((item) => item === values["SpawnPoints"]) ===
                            -1) {
                    ...
                    for (let [_, values] of Object.entries(this.defaultSpawnParms[map])) {
                        ...
                        if (values["Infiltration"] !== undefined &&
                            values["Infiltration"].length > 1 &&
                            infiltrationsArr.findIndex((item) => item === values["Infiltration"]) === -1) {
                            infiltrationsArr.push(values["Infiltration"]);
                        }
                        // Spawn Zone
                        if (values["BotZoneName"] !== undefined &&
                    ...
                    if (lists.length > 0) {
                        ...
                        logger.log(`${this.Name}: Config Created "${filePath.replace("user/mods/Lua-CustomSpawnPoints/", "")}" `, LogTextColor_1.LogTextColor.YELLOW, LogBackgroundColor_1.LogBackgroundColor.BLUE);
                    }
                    else {
                        continue;
                    }
                    ...
                    for (let [zoneName, values] of Object.entries(config)) {
                        if (zoneName === "SpawnZone Available List" ||
                        ...
                        for (let index in config[zoneName]) {
                            ...
                            if (zoneConfig.Position === undefined ||
                            ...
                            if (zoneConfig.Rotation === undefined ||
                            ...
                            if (zoneConfig.DelayToCanSpawnSec === undefined ||
                            ...
                            if (zoneConfig.ColliderRadius === undefined ||
                            ...
                            if (config_json_1.default.DEBUG.PrintAddedSpawnPoints === true) {
            ...
            for (let [_, values] of Object.entries(newSpawnPointParams)) {
                if (values.Categories.includes("Bot") === true)
                    foundSpawn[0] = true;
                if (values.Categories.includes("Player") === true)
                    foundSpawn[1] = true;
                if (foundSpawn.includes(false) === false)
                    break;
            }
            // Check missing spawn points from default spawn points
            if (foundSpawn.includes(false) === true) {
                if (foundSpawn[0] === false)
                    logger.error(`${this.Name}: No spawn point for "Bot" in "${map}"... `);
                if (foundSpawn[1] === false)
                    logger.error(`${this.Name}: No spawn point for "Player" in "${map}"... `);
                for (let key in this.defaultSpawnParms[map]) {
                    ...
                    for (let newKey in newSpawnPointParams) {
                        if (this.defaultSpawnParms[map][key].Id ===
                    ...
                    if (currentSpawnPointParms === undefined) {
                        ...
                        if (spawnPoint !== undefined)
                            newSpawnPointParams.push(spawnPoint);
                    }
                    else {
        ...
        if (updatedList.length > 0) {
            logger.log(`${this.Name}: Custom Spawn Points    Bot: ${totalSpawnPoints.Custom.Bot},${this.GetSpace(totalSpawnPoints.Custom.Bot, 5)}Player: ${totalSpawnPoints.Custom.Player},${this.GetSpace(totalSpawnPoints.Custom.Player, 5)}Boss: ${totalSpawnPoints.Custom.Boss} `, LogTextColor_1.LogTextColor.WHITE, LogBackgroundColor_1.LogBackgroundColor.BLUE);
            logger.log(`${this.Name}: Default Spawn Points   Bot: ${totalSpawnPoints.Default.Bot},${this.GetSpace(totalSpawnPoints.Default.Bot, 5)}Player: ${totalSpawnPoints.Default.Player},${this.GetSpace(totalSpawnPoints.Default.Player, 5)}Boss: ${totalSpawnPoints.Default.Boss} `, LogTextColor_1.LogTextColor.WHITE, LogBackgroundColor_1.LogBackgroundColor.BLUE);
            logger.log(`${this.Name}: Successfully Loaded ${dirsList.length - 1} Presets for "${updatedList}" `, LogTextColor_1.LogTextColor.WHITE, LogBackgroundColor_1.LogBackgroundColor.BLUE);
        }
        else {
            logger.log(`${this.Name}: Default Spawn Points   Bot: ${totalSpawnPoints.Default.Bot},${this.GetSpace(totalSpawnPoints.Default.Bot, 5)}Player: ${totalSpawnPoints.Default.Player},${this.GetSpace(totalSpawnPoints.Default.Player, 5)}Boss: ${totalSpawnPoints.Default.Boss} `, LogTextColor_1.LogTextColor.WHITE, LogBackgroundColor_1.LogBackgroundColor.BLUE);
        }
        if (readMe) {
            logger.log(`${this.Name}: Check out "!Readme/example.json" for how to make Custom Spawn Points! `, LogTextColor_1.LogTextColor.WHITE, LogBackgroundColor_1.LogBackgroundColor.MAGENTA);
        }
    }
    GetSpace(str, line) {
